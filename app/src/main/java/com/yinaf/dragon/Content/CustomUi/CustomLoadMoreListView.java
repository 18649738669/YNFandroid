 ...
 * 功能：配合CustomRefreshAndLoadMoreView 完成下拉刷新、滑到底部自动加载更多
 */
public class CustomLoadMoreListView extends ListView implements AbsListView.OnScrollListener {
    ...
    private CustomRefreshAndLoadMoreView mCustomRefreshAndLoadMoreView;
    private boolean isLoading = false;// 是否正在加载

    private OnLoadMoreListener mOnLoadMoreListener;

    public CustomLoadMoreListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //动态载入底部布局
    ...
    public void setRefreshAndLoadMoreView(CustomRefreshAndLoadMoreView mCustomRefreshAndLoadMoreView) {
        this.mCustomRefreshAndLoadMoreView = mCustomRefreshAndLoadMoreView;
    }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
            if (view.getLastVisiblePosition() == view.getCount() - 1 && (mCustomRefreshAndLoadMoreView != null &&
                    !mCustomRefreshAndLoadMoreView.isRefreshing()) && !isLoading && mOnLoadMoreListener != null && isHaveMoreData) {
                isLoading = true;
