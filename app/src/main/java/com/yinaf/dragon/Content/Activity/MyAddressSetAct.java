...
import com.yinaf.dragon.Content.Adapter.MyAddressSetAdapter;
import com.yinaf.dragon.Content.Bean.MyAddressSetModel;
import com.yinaf.dragon.Content.Dialog.LoadingDialog;
import com.yinaf.dragon.Content.Net.MyAddressSetAPI;
 ...
 * 用户地址 - 地址管理列表
 */
public class MyAddressSetAct extends BasicAct implements MyAddressSetAPI.AddressSetListener {

    @BindView(R.id.recycler_view)
    RecyclerView recyclerView;
    @BindView(R.id.tv_empty)
    TextView tv_empty;
    private MyAddressSetAdapter drugSetAdapter;
    private String userId;
    private LoadingDialog loadingDialog;

    public MyAddressSetAct() {
    ...
    public void initView() {
        ...
        if (!NetworkUtils.isNetworkConnect(getApplicationContext())){
            String string = SPHelper.getString(Builds.SP_USER, userId + "MyAddress");
            if (string!=null&&!TextUtils.isEmpty(string)){
                MyAddressSetModel   model = JSONUtils.parseJson(string, MyAddressSetModel.class);
                drugSetAdapter = new MyAddressSetAdapter(getApplicationContext()
                        , this,model.getObj());
            ...
            loadingDialog.show();
            new MyAddressSetAPI(this, userId);
        }
    ...
    public void addressSetSuccess(MyAddressSetModel drugSetModel) {
        List<MyAddressSetModel.ObjBean> obj = drugSetModel.getObj();

        if (obj.size() > 0) {
            if (drugSetAdapter == null) {
                drugSetAdapter = new MyAddressSetAdapter(getApplicationContext(), this, obj);
                recyclerView.setAdapter(drugSetAdapter);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.tool_bar_friends_circle_right_text:
                //添加地址
                Intent intent = new Intent(getApplicationContext(), MyAddressAddSetAct.class);
